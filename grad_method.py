...
def E(y, a, b): # функция для ошибок
    ff = np.array([a * z + b for z in range(N)])  # теоритическая прямая
    return np.dot((y - ff).T, (y - ff))  # Y - экспериментальное значение, ff - значение прямой ff.


def dEda(y, a, b):  # вычисление частных производных
    ff = np.array([a * z + b for z in range(N)])
    return -2 * np.dot((y - ff).T, range(N)) # range(N) - x-итое


def dEdb(y, a, b):  # вычисление частных производных
    ff = np.array([a * z + b for z in range(N)])
    return -2 * (y - ff).sum()
